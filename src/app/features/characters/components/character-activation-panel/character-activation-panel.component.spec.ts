...
import { CharacterActivationPanelComponent } from './character-activation-panel.component';
...
describe('CharacterActivationPanelComponent', () => {
  let component: CharacterActivationPanelComponent;
  let charService: {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      imports: [CharacterActivationPanelComponent],
      providers: [{ provide: CharacterService, useValue: charService }],
    });
    TestBed.overrideComponent(CharacterActivationPanelComponent, { set: { template: '', imports: [] } });

    component = TestBed.createComponent(CharacterActivationPanelComponent).componentInstance;
  });
  ...
  describe('activate', () => {
    ...
    it('calls activateCharacters with selected ids and emits activated on success', () => {
      const activatedSpy = vi.fn();
      component.activated.subscribe(activatedSpy);
      component.selectedIds.set(new Set([1, 2]));
    ...
    it('resets isActivating and emits activateError on failure', () => {
      const errorSpy = vi.fn();
      component.activateError.subscribe(errorSpy);
      component.selectedIds.set(new Set([1]));
  ...
  describe('openSync', () => {
    it('emits openSyncRequested', () => {
      const openSyncSpy = vi.fn();
      component.openSyncRequested.subscribe(openSyncSpy);
