...
import { GuildAttributionDefinition, GuildAttributionDefinitionPayload } from '../../../raids/models/guild-attribution-definition.model';
...
export class AttributionDefinitionDialogComponent {
  ...
  onSpellSelected(key: string, spell: Spell): void {
    ...
    if (!this.label().trim()) this.label.set(spell.name);
  }
  ...
  submit(): void {
    ...
    const request = this.data.definition
      ...
      : this.#definitionsService.createDefinition(this.data.guildId, payload);
